...
const HeaderRecruiter = ({toggleSidebar,isSideBarOpen}) => {
  ...
  return (
    <div className="w-full fixed top-0 bg-white z-50 border-none">
       <div className='max-w-[1024px] mx-auto pt-[20px] pb-[24px] flex font-urbanist items-center justify-between h-[88px] px-6 bg-white z-50 border-none'>
        {/* left part */}
      <div onClick={handleSetup} className="cursor-pointer flex gap-[12px] items-center">
        <div>
            <img src={images.google} className="h-[38.4px] rounded-lg border border-blue-600" alt="" />
        </div>
        <div className='flex flex-col '>
          <span className='text-[12px] font-medium leading-[20px] text-custGrey' >Welcome back Google</span>
          <span className='inline-flex text-[14px] font-bold leading-[22px] items-center gap-[4px]'>Manage your hiring in one place </span>
          {/* <span className='inline-flex text-[18px] font-bold leading-[22px] items-center gap-[4px]'>Manage your hiring in one place </span> */}
        </div>
      </div>
      ...
      className="flex gap-[10px] items-center">
        <div onClick={handleNotification} className='h-[40px] w-[40px] p-2 rounded-lg bg-gray-100 cursor-pointer hover:bg-blue-500 transition-all group'>
