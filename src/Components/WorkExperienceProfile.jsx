...
const Header = ({ onBack, onAdd }) => (
  <div className="fixed top-0 left-0 right-0 bg-white z-10 dark:bg-[#111d39]">
    <div className="flex items-center justify-between max-w-[1024px] mx-auto px-[24px] pt-[16px] pb-[24px]">
      ...
      <h2 className="text-[20px] font-bold leading-[24px] ml-6 dark:text-white">Work Experience</h2>
...
const ExperienceCard = ({ experience, onEdit, onRemove }) => {
  const { job_title, company_name, Department, location, start_date, end_date, currently_working, description } = experience;
  return (
    <div className="bg-white dark:bg-[#242f49] rounded-xl p-[18px] border border-gray-200 dark:border-gray-800 shadow-md">
      <div className="flex items-start justify-between mb-[16px]">
        <div className="flex-1 space-y-[6px]">
          <h3 className="text-[18px] font-bold leading-[22px] text-[#121927] dark:text-white">{job_title || "N/A"}</h3>
          <p className="text-[14px] text-gray-700 dark:text-gray-300 font-medium">{company_name}</p>
          <p className="text-[13px] text-gray-500 dark:text-gray-400">{Department} - {location}</p>
          <p className="text-[13px] text-gray-600 dark:text-gray-300">
            {start_date} - {currently_working ? "Present" : end_date}
          </p>
          {description && <p className="text-[13px] font-medium text-[#121927]/80 dark:text-gray-300">{description}</p>}
        </div>

        <div className="flex gap-[8px] ml-[12px]">
          <button onClick={() => onEdit(experience)} className="p-[8px] bg-[#E5FAF5] dark:bg-[#313b54] rounded-lg hover:bg-[#d1f5e8] dark:hover:bg-[#3a4a6a]">
            <MdEdit className="size-[16px] text-[#00CC9A]" />
          </button>
          <button onClick={() => onRemove(experience.id)} className="p-[8px] bg-[#FFEFF8] dark:bg-[#313b54] rounded-lg hover:bg-[#ffe0f0] dark:hover:bg-[#3a4a6a]">
...
const EmptyState = ({ onAdd }) => (
  <div className="text-center py-[48px]">
    ...
    <button onClick={onAdd} className="bg-[#2869FE] text-white px-[16px] py-[8px] rounded-xl text-[14px] font-semibold flex items-center gap-[8px] mx-auto">
      <FaPlus className="size-[12px]" />
      Add Experience
    </button>
  </div>
...
const SkeletonLoader = () => (
  <div className="max-w-[1024px] mx-auto px-[24px] pt-[80px]">
    <div className="space-y-[16px]">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="bg-white dark:bg-[#242f49] rounded-xl p-[18px] border border-gray-200 dark:border-gray-800 shadow-md animate-pulse">
          <div className="flex items-start justify-between mb-[16px]">
...
const WorkExperienceProfile = () => {
  ...
  useEffect(() => {
    const fetchExperienceData = async () => {
      try {
        setLoading(true);
        const res = await api.get("/dashboards/jobseeker/experiences/");
        console.log('work',res.data.data[0]);
  ...
  const handleAdd = () => navigate("/workexperience/add");
  const handleEdit = (experience) => navigate(`/workexperience/edit/${experience.id}`);


const handleRemove = async (id) => {
  const result = await Swal.fire({
    title: 'Are you sure?',
    text: "You want to delete this work experience?",
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      await api.delete(`/dashboards/experiences/${id}/update/`);
